...
def create_initial_grid(rows, cols):
    ...
    ring_from_hashed_image_number = create_ring_from_hashed_image()

    grid = []
    for row in range(rows):
        grid_rows = []
        for col in range(cols):
            # Get the next number from the hash ring
            next_generated_number = next(ring_from_hashed_image_number)
            if  next_generated_number == 0 or next_generated_number == 5 or next_generated_number == 9:
                grid_rows += [1]
            else:
                grid_rows += [0]
        grid += [grid_rows]
...
def create_ring_from_hashed_image():

    """
    Creates a ring which is a list that can iterate over it self again and again
    like a ring. The list is a string of numbers, numbers which were generated by 
    aplying a hash function over an image file.
    ...
    return hash_calculator.infinite_ring(hash)
